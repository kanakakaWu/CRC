...
//--CONSTANT  residue:  vlbit_1d(31 downto 0) := "11000111000001001101110101111011"; -- C704DD7B residue bit flip
//CONSTANT  residue:  vlbit_1d(31 downto 0) :=   "11011110101110110010000011100011"; -- DEBB20E3 residue
...
static const uint32_t crc32_ref_tab[] = {
...
uint32_t crc32(uint32_t crc_in, void *buf, int size)
{
	uint8_t *p;
	...
	p = (uint8_t *)buf;
	crc = crc_in ^ 0xFFFFFFFF;


	while (size--)
	{
		crc = crc32_ref_tab[(crc ^*p++ ) & 0xFF] ^ (crc >> 8);
	}
...
uint32_t crc32_check(void *buf,int size)
{
 	uint32_t crc;
  	crc = crc32(0, buf, size);

	return 	0xDEBB20E3 - (crc^0xFFFFFFFF);

}

void add_crc32(void *buf,int size)
{
	uint32_t crc;
	uint8_t *p,*d;
...
unsigned char example_packet[64] = {
...
void main()
{
	...
	uint32_t check_res = 0x01;
	//uint32_t xdata res1 = 0x02;
    

	add_crc32(example_packet, 60);

	check_res = 0x03;
	check_res = crc32_check(example_packet, 64);

	add_crc32(frame, 9);

	check_res = 0x0A;
	check_res = crc32_check(frame, 13);
